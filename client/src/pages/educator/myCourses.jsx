      ...
      <div className='w-full'>
        ...
        <div className='flex flex-col items-center max-w-4xl w-full overflow-hidden rounded-md bg-white border border-gray-600/20'>
          <table className='md:table-auto table-fixed w-full overflow-hidden'>
            <thead className='text-gray-600 border-b border-gray-400/20 text-sm text-left'>
              <tr>
            ...
            <tbody className='text-sm text-gray-400'>
              {courses.map((course) => (
                <tr key = {course._id} className='border-b border-gray-800/20'>
                  <td className='md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3 truncate'>
                    <img src={course.courseThumbnail} alt = "" className='w-16'/>
                    <span className='truncate text-gray-900 hidden md:block'>{course.courseTitle}</span>
                  </td>
                  <td className='px-4 py-3'>
                      {currency}
                      {Math.floor(
                      course.enrolledStudents.length *
                      (   course.coursePrice - (course.courseDiscount || 0) * course.coursePrice / 100)
                    )}
                  </td>
